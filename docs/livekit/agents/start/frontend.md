...
您應該使用聲音效果、觸覺回饋和視覺效果來讓您的代理感覺更靈敏。這在長時間思考狀態下尤其重要（例如，執行外部尋找或使用工具時）。[視覺化工具](#audio-visualizer) 包含基本的 `thinking` 狀態指示，並且還允許使用者注意到他們的音訊何時無法運作。如需更進階的效果，請使用 狀態與控制 功能在前端觸發效果。