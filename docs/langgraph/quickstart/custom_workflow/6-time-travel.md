...
在典型的聊天機器人工作流程中，使用者需要與機器人進行一次或多次互動才能完成一項任務。Memory 和 human-in-the-loop 機制會在 graph 狀態中啟用檢查點，並控制未來的反應。

如果您希望用戶能夠從先前的回覆開始，探索不同的結果，該怎麼辦？或者，如果您希望使用者能夠回放聊天機器人的工作以修復錯誤或嘗試不同的策略（這在自主軟體工程師等應用程式中很常見），該怎麼辦？

您可以使用 LangGraph 的內建 **time travel** 功能來建立這些類型的體驗。

## 1. Rewind your graph

使用 graph 的 `get_state_history` 方法取得檢查點，即可回溯 graph。然後，您可以從該時間點恢復執行。
...
在 graph 中新增步驟。每個步驟都會在其狀態歷史記錄中設定檢查點：
...
現在您已經為聊天機器人添加了步驟，您可以重播完整的狀態歷史記錄以查看發生的所有事情。
...
檢查點會儲存 graph 中每一步的執行情況。這涵蓋了所有調用，因此您可以回溯整個線程的歷史記錄。

## Resume from a checkpoint

從 `to_replay` 狀態恢復，該狀態位於第二次 graph 呼叫中的聊天機器人節點之後。從此處恢復將呼叫接下來的操作節點。
...
```bash
('tools',)
{'configurable': {'thread_id': '1', 'checkpoint_ns': '', 'checkpoint_id': efd43e3-0c1f-6c4e-8006-891877d65740'}}
```

## 4. Load a state from a moment-in-time

檢查點的 `to_replay.config` 檔案包含一個 `checkpoint_id` 時間戳記。提供此 `checkpoint_id` 值會告知 LangGraph 的檢查點程式從該時刻開始載入狀態。
...
Graph 從操作節點恢復執行。您可以判斷情況確實如此，因為上面列印的第一個值是來自我們的搜尋引擎工具的回應。

恭喜！現在您已經在 LangGraph 中使用了時間旅行檢查點遍歷。能夠回溯並探索其他路徑，為調試、實驗和互動式應用程式開闢了無限可能。