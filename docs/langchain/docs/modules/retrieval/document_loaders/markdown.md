...
> [Markdown](https://en.wikipedia.org/wiki/Markdown) 是一種輕量級標記語言，用於使用純文字編輯器建立格式化文字。

這涵蓋如何將 `Markdown` 文件載入為我們可以在下游應用程式可使用的文件格式。
...
在幕後，[unstructured](https://pypi.org/project/unstructured/) 套件為不同的文字區塊創建不同的 `elements`。預設情況下，我們將它們組合在一起，但您可以透過指定 `mode="elements"` 輕鬆保持這種分離。
